import sys, subprocess, importlib, io
import streamlit as st
import pandas as pd

# Темная тема с фиолетовыми акцентами
st.set_page_config(page_title="Генератор отчётов по времени", page_icon="⏱️", layout="wide")
st.markdown("""
    <style>
    .stApp { background-color: #2b2b2b; color: #f0f0f0; }
    h1, h2, h3, h4 { color: #9b59b6 !important; }
    .stMarkdown p, .stText, .stDataFrame { color: #f0f0f0 !important; }
    .stButton>button, .stDownloadButton>button {
        background-color: #9b59b6; color: #ffffff; border: none; border-radius: 8px; padding: 0.6rem 1rem;
    }
    .stButton>button:hover, .stDownloadButton>button:hover { background-color: #884ea0; color: #ffffff; }
    section[data-testid="stSidebar"] { background-color: #1e1e1e; }
    .stProgress>div>div>div>div { background-color: #9b59b6; }
    </style>
""", unsafe_allow_html=True)

# Проверка/установка openpyxl
def ensure_openpyxl(version="3.1.5"):
    try:
        import openpyxl
        return True
    except ImportError:
        try:
            subprocess.check_call([sys.executable, "-m", "pip", "install", f"openpyxl=={version}"])
            importlib.invalidate_caches()
            import openpyxl
            return True
        except Exception as e:
            st.error(f"Не удалось установить openpyxl: {e}")
            return False

st.sidebar.header("Проверка зависимостей")
if ensure_openpyxl():
    st.sidebar.success("✅ openpyxl установлен")
else:
    st.stop()

# Основной интерфейс
st.title("Генератор отчётов по времени")
st.write("Загрузите Excel-файл и получите сводный отчёт по проектам и специалистам.")

uploaded_file = st.file_uploader("Перетащите файл .xlsx или выберите его", type=["xlsx"])

def process_timesheet(df):
    if not all(col in df.columns for col in ['Имя активности', 'Полное название', 'Записанные часы']):
        st.error("Файл должен содержать столбцы: 'Имя активности', 'Полное название', 'Записанные часы'")
        return None
    df['Записанные часы'] = pd.to_numeric(df['Записанные часы'], errors='coerce').fillna(0)
    result = df.groupby(['Имя активности','Полное название'])['Записанные часы'].sum().reset_index()
    result.columns = ['Проект','Специалист','Часы']
    return result

if uploaded_file is not None:
    try:
        df = pd.read_excel(uploaded_file, engine='openpyxl')
        st.success("Файл успешно загружен!")
        result = process_timesheet(df)
        if result is not None:
            st.subheader("Предварительный просмотр")
            st.dataframe(result.head(10))

            @st.cache_data
            def to_excel(dataframe):
                buffer = io.BytesIO()
                with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                    dataframe.to_excel(writer, index=False)
                return buffer.getvalue()

            excel_data = to_excel(result)
            st.download_button("Скачать отчёт в Excel", excel_data, "report.xlsx")
    except Exception as e:
        st.error(f"Ошибка при обработке: {e}")
